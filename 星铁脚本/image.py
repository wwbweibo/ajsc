...
def check_game_status(image, actions):
    for action in actions:
        action_name = action[0]
        pic = action[1]
        func = action[2]
        width, height = pic.shape[::-1]
        res = cv2.matchTemplate(image, pic, cv2.TM_CCOEFF_NORMED)
        min_val, max_val , min_loc, max_loc = cv2.minMaxLoc(res)
        # print("test action %s with max_val %f" % (action_name, max_val))
        if max_val > 0.8:
            x, y = max_loc
            return action_name, func, x + (width / 2), y + (height / 2)
    return None
